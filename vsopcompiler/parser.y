...
class:
    CLASS TYPE_IDENTIFIER classBody { $$ = std::make_shared<ClassNode>(@1, $2, $3); }
    | CLASS TYPE_IDENTIFIER EXTENDS TYPE_IDENTIFIER classBody { $$ = std::make_shared<ClassNode>(@1, $2, $5, $4); };
...
field:
    OBJECT_IDENTIFIER COLON type SEMICOLON { $$ = std::make_shared<FieldNode>(@1, $1, $3); }
    | OBJECT_IDENTIFIER COLON type ASSIGN expr SEMICOLON { $$ = std::make_shared<FieldNode>(@1, $1, $3, $5); };

method:
    OBJECT_IDENTIFIER LPAR formals RPAR COLON type block { $$ = std::make_shared<MethodNode>(@1, $1, $3, $6, $7); };
...
formal:
    OBJECT_IDENTIFIER COLON type { $$ = std::make_shared<FormalNode>(@1, $1, $3); };

block:
    LBRACE exprList RBRACE { $$ = std::make_shared<BlockNode>(@1, $2); };
...
expr:
    ...
    | callExpr { $$ = $1;}
    | newExpr { $$ = $1;}
    | OBJECT_IDENTIFIER { $$ = std::make_shared<IdentifierNode>(@1, $1); }
    | SELF { $$ = std::make_shared<SelfNode>(@1); }
    | literal { $$ = $1; }
    | LPAR RPAR { $$ = std::make_shared<UnitNode>(@1); }
    | LPAR expr RPAR { $$ = $2; }
    | block { $$ = $1; };
...
ifExpr:
    IF expr THEN expr { $$ = std::make_shared<IfNode>(@1, $2, $4); }
    | IF expr THEN expr ELSE expr { $$ = std::make_shared<IfNode>(@1, $2, $4, $6); }

whileExpr:
    WHILE expr DO expr { $$ = std::make_shared<WhileNode>(@1, $2, $4); };

letExpr:
    LET OBJECT_IDENTIFIER COLON type IN expr { $$ = std::make_shared<LetNode>(@1, $2, $4, $6); }
    | LET OBJECT_IDENTIFIER COLON type ASSIGN expr IN expr { $$ = std::make_shared<LetNode>(@1, $2, $4, $8, $6); };

assignExpr:
    OBJECT_IDENTIFIER ASSIGN expr { $$ = std::make_shared<AssignNode>(@1, $1, $3); };

unOpExpr:
    NOT expr { $$ = std::make_shared<NotUnOpNode>(@1, $2); }
    | MINUS expr %prec UNARYMINUS { $$ = std::make_shared<MinusUnOpNode>(@1, $2); }
    | ISNULL expr { $$ = std::make_shared<IsnullUnOpNode>(@1, $2); };
...
callExpr:
    OBJECT_IDENTIFIER LPAR args RPAR { $$ = std::make_shared<CallNode>(@1, $1, $3); }
    | expr DOT OBJECT_IDENTIFIER LPAR args RPAR { $$ = std::make_shared<CallNode>(@1, $3, $5, $1); };

newExpr:
    NEW TYPE_IDENTIFIER { $$ = std::make_shared<NewNode>(@1, $2);};

literal:
    INTEGER_LITERAL { $$ = std::make_shared<LiteralNode>(@1, std::to_string($1), "int32"); }
    | STRING_LITERAL { $$ = std::make_shared<LiteralNode>(@1, $1, "string"); }
    | booleanLiteral { $$ = $1; };

booleanLiteral:
    TRUE { $$ = std::make_shared<LiteralNode>(@1, "true", "bool"); }
    | FALSE { $$ = std::make_shared<LiteralNode>(@1, "false", "bool"); };
